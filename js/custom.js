		...
		$('.requiredField').each(function() {
			if(jQuery.trim($(this).val()) == '') {
            	...
            	if(!emailReg.test(jQuery.trim($(this).val()))) {
            		var labelText = $(this).prev('label').text();
            		$(this).parent().append('<span class="error">Has introducido un '+labelText+' incorrecto.</span>');
            		$(this).addClass('inputError');
            		hasError = true;
            	}
            }
		...
		if(!hasError) {
			...
			$.post($(this).attr('action'),formInput, function(data){
				$('form#contact_form').slideUp("fast", function() {
					$(this).before('<p class="success">Gracias! Tu correo ha sido enviado satisfactoriamente</p>');
				});
			});
		}
...
function ScrollToBottom() {
	...
	//$("html, body").animate({ scrollTop: $("footer_wrapper").scrollTop() }, 1000);
}
